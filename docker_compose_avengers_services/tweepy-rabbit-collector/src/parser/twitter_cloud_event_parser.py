...
class TwitterEventToCloudEvent(EventParser):    
    ...
    def extract_twitter_data(self, event=None):
        ...
        return (_id, parsed_data)

    def cloud_event(self, id=None, event_body=None):
        event = (
            v02.Event().
                SetContentType("application/json").
                SetData(event_body).
                SetEventID(id).
                SetSource("avengers.itau.collectors.tweepy").
    ...
    def parse_event(self,event=None):
        _id, data = self.extract_twitter_data(event)
        return (_id, self.cloud_event( id=_id, event_body=data)) 